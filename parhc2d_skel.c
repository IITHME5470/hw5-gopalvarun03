...
void enforce_bcs(int nx, int ny, double *x, double *y, double **T, int nxglob, int nyglob, int istglob, int ienglob, int jstglob, int jenglob)
{
  int i, j;
  ...
  for (i = 0; i < nx; i++)
  {
    if (jstglob == 0) // processors adjacent to the bottom end of the domain
      T[i][0] = 0.0;
    // T[i][j-1] replaced with T[i][0]

    if (jenglob == nyglob - 1) // processors adjacent to the top end of the domain
      T[i][ny - 1] = 0.0;
    // T[i][j+1] replaced with T[i][ny-1]
  }
}

void set_initial_condition(int nx, int ny, double *x, double *y, double **T, double dx, double dy, int nxglob, int nyglob, int istglob, int ienglob, int jstglob, int jenglob)
{
  int i, j;
  double del = 1.0;
  ...
  enforce_bcs(nx, ny, x, y, T, nxglob, nyglob, istglob, ienglob, jstglob, jenglob); // ensure BCs are satisfied at t = 0
}
...
void get_rhs(int nx, int nxglob, int ny, int nyglob, int istglob, int ienglob, int jstglob, int jenglob, double dx, double dy, double *xleftghost, double *xrightghost, double *ybotghost, double *ytopghost, double kdiff, double *x, double *y, double **T, double **rhs)
{
  ...
  i = nx - 1;
  if (ienglob == nxglob - 1)
    for (j = 1; j < ny - 1; j++) // processors adjacent to the right end of the domain
      rhs[i][j] = 0.0;
  else
    for (j = 1; j < ny - 1; j++)
  ...
  j = 0;
  if (jstglob == 0)
    for (i = 1; i < nx - 1; i++) // processors adjacent to the bottom end of the domain
      rhs[i][j] = 0.0;
  else
    for (i = 1; i < nx - 1; i++)
      rhs[i][j] = kdiff * (T[i + 1][j] + T[i - 1][j] - 2.0 * T[i][j]) / dxsq +
  ...
  i = nx - 1;
  j = ny - 1;
  if (ienglob == nxglob - 1 || jenglob == nyglob - 1) // processors adjacent to the left or top ends of the domain
    rhs[i][j] = 0.0;
  else
    rhs[i][j] = kdiff * (T[i - 1][j] + xrightghost[j] - 2.0 * T[i][j]) / dxsq +
                kdiff * (T[i][j - 1] + ytopghost[i] - 2.0 * T[i][j]) / dysq;
...
void timestep_FwdEuler(int rank, int size, int rank_x, int rank_y, int px, int py, int nx, int nxglob, int ny, int nyglob, int istglob, int ienglob, int jstglob, int jenglob, double dt, double dx, double dy, double *xleftghost, double *xrightghost, double *ybotghost, double *ytopghost, double kdiff, double *x, double *y, double **T, double **rhs, double *sendbuf_x, double *recvbuf_x, double *sendbuf_y, double *recvbuf_y)
{
  ...
  // set Dirichlet BCs
  enforce_bcs(nx, ny, x, y, T, nxglob, nyglob, istglob, ienglob, jstglob, jenglob);
}
...
void timestep_BwdEuler(int nx, int ny, double dt, double dx, double dy, double kdiff, double *x, double *y, double **T, double **rhs, double **Tnew, int nxglob, int nyglob, int istglob, int ienglob, int jstglob, int jenglob)
{
...
int main(int argc, char **argv)
{
  ...
  set_initial_condition(nx, ny, x, y, T, dx, dy, nxglob, nyglob, istglob, ienglob, jstglob, jenglob); // initial condition
